...
const FILTER_FIELD = 'country' as const;
...
export function CountryAllocationChart({ assets, activeFilter, onFilterChange }: Props) {
  const map = new Map<string, number>();
  for (const asset of assets) {
    map.set(asset.country, (map.get(asset.country) ?? 0) + asset.value);
  }
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-5 border border-gray-700">
      <h2 className="text-base font-semibold text-white mb-1">Alocação por País</h2>
          ...
          >
            {data.map((entry) => {
              ...
              return (
                <Cell
                  key={entry.name}
                  fill={COUNTRY_COLORS[entry.name] ?? FALLBACK_COLOR}
